...
int stage = 0; //counter for stage
int x = 0; //counter for all black
int y = 0; //counter for all white
        ...
        if (bt_3_S_Time_Up)
        {
          ...
          switch (stage) {
            case 0: {
                ...
                if (encoder_LeftMotor.getRawPosition() < -990 && encoder_RightMotor.getRawPosition() < -990)
                {
                  encoder_LeftMotor.zero();
                  encoder_RightMotor.zero();
                  stage = 1;
              ...
              break;
            case 1: { //turn until tracks line
                ui_Left_Motor_Speed = 0;
                ui_Right_Motor_Speed = 1300;
                if (encoder_RightMotor.getRawPosition() < -1200) {
                  stage = 2;
                  encoder_LeftMotor.zero();
                  encoder_RightMotor.zero();
                }
              ...
              break;
            case 2: {//tracks line until needs to turn
                Drive(1650);
                if (encoder_LeftMotor.getRawPosition() > 3330 && encoder_RightMotor.getRawPosition() > 2640)
                {
                  encoder_LeftMotor.zero();
                  encoder_RightMotor.zero();
                  stage = 3;
              ...
              break;
            case 3: { //turns left until catches line
                ui_Left_Motor_Speed = 0;
                ui_Right_Motor_Speed = 1600;
                if (encoder_RightMotor.getRawPosition() > 620) {
                  stage = 4;
                  encoder_LeftMotor.zero();
                  encoder_RightMotor.zero();
                }
            ...
            case 5: {
                ui_Left_Motor_Speed = 1300;
                ui_Right_Motor_Speed = 1300;
                if (encoder_LeftMotor.getRawPosition() < -940 && encoder_RightMotor.getRawPosition() < -940)
                {
                  encoder_LeftMotor.zero();
                  encoder_RightMotor.zero();
                  stage = 6;
            ...
            case 6: {
                ...
                if (encoder_LeftMotor.getRawPosition() < -1180) {
                  stage = 7;
                  encoder_LeftMotor.zero();
                  encoder_RightMotor.zero();
                }
            ...
            case 7: {
                Drive(1650);
                if (encoder_LeftMotor.getRawPosition() > 3100 && encoder_RightMotor.getRawPosition() > 3500)
                {
            ...
            case 8: {
                ...
                ui_Right_Motor_Speed = 1600;
                if (encoder_LeftMotor.getRawPosition() > 600 && encoder_RightMotor.getRawPosition() > 600)
                {
                  ui_Left_Motor_Speed = 1500;
                  ui_Right_Motor_Speed = 1500;
                  encoder_LeftMotor.zero();
              ...
              break;
            case 9: {
                lightSens = analogRead(ci_Light_Sensor);
                Serial.println(lightSens);
                if (encoder_LeftMotor.getRawPosition() < 80) {
                  ui_Left_Motor_Speed = 1600;
            ...
            case 10: {
                lightSens = analogRead(ci_Light_Sensor);
                Serial.println(lightSens);
                if (encoder_LeftMotor.getRawPosition() > -170) {
...
void CloseClaw () {
  servo_GripMotor.write(30);
}

void ExtendArm () {
  for (int d = 7; d <= 13; d++) {
    servo_ArmMotor.write(d * 10);
    delay(100);
  }
}
